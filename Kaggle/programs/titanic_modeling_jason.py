...
pd.crosstab(train_df.name_title, train_df.Survived).plot(kind = 'bar')

# Title to Profession_Type mapping
train_df['name_title'] = train_df.Name.apply(lambda x: x.split(',')[1].split('.')[0].strip())

if train_df['name_title'].str.extract(r'(Dr|Rev)').notna:
    train_df['profession_type'] = "Professional"
else: train_df['profession_type'] = "Other"

train_df["working"] = train_df["name_title"].str.extract(r'(Mr|Mrs|Miss|Mlle|Ms)')
train_df["professional"] = train_df["name_title"].str.extract(r'(Dr|Rev)')
train_df["military"] = train_df["name_title"].str.extract(r'(Col|Major|Capt)')
train_df["royalty"] = train_df["name_title"].str.extract(r'(Master|Sir|Lady|Mme|Don|Jonkheer|the Countess)')
train_df.groupby('working').mean()
train_df.groupby('professional').mean()
train_df.groupby('military').mean()
train_df.groupby('royalty').mean()
pd.crosstab(train_df.working, train_df.Survived).plot(kind = 'bar')
pd.crosstab(train_df.royalty, train_df.Survived).plot(kind = 'bar')
pd.crosstab(train_df.military, train_df.Survived).plot(kind = 'bar')
pd.crosstab(train_df.professional, train_df.Survived).plot(kind = 'bar')

train_df["profession_type"] = train_df["working"], train_df["professional"], train_df["military"], train_df["royalty"]
