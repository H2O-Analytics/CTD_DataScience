...
tweets_list_southwest = []

# Using TwitterSearchScraper to scrape data and append tweets to list
for i,tweet in enumerate(sntwitter.TwitterSearchScraper('@SouthwestAir since:2022-12-14 until:2022-12-21').get_items()):
    if i>20:
        break
    tweets_list_southwest.append([tweet.date, tweet.id, tweet.content, tweet.user.username])

# Create a list to append tweet data
tweets_list_southwest2 = []
maxTweets = 10000

# Using TwitterSearchScraper to scrape data and append tweets to list
for i,tweet in enumerate(sntwitter.TwitterSearchScraper('@SouthwestAir since:2022-01-01 until:{today}').get_items()):
    if i>maxTweets:
        break
    tweets_list_southwest2.append([tweet.content])
    
# Creating a dataframe from the tweets list above
tweets_to_df = pd.DataFrame(tweets_list_southwest2, columns=['Tweets'])
...
tweets_to_df.to_csv('tweets_southwest.csv') #write dataframe into csv file
savedTweets = pd.read_csv('tweets_southwest.csv',index_col=0) #reads csv file
...
for tweets_to_df in tweets_to_df:
    score = sia.polarity_scores(tweets_to_df)["compound"]
    print(f'The sentiment value of the tweet :"{tweets_to_df}" is : {score}')

for tweets_to_df in tweets_to_df:
    print(f'For the tweet "{tweets_to_df}"')
    polarity = sia.polarity_scores(tweets_to_df)
    pos = polarity["pos"]
    neu = polarity["neu"]
    neg = polarity["neg"]
    print(f'The percententage of positive sentiment in :"{tweets_to_df}" is : {round(pos*100,2)} %')
    print(f'The percententage of neutral sentiment in :"{tweets_to_df}" is : {round(neu*100,2)} %')
    print(f'The percententage of negative sentiment in :"{tweets_to_df}" is : {round(neg*100,2)} %')
    print("="*50)