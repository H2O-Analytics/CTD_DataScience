...
for i,tweet in enumerate(sntwitter.TwitterSearchScraper('bitcoin since:2022-12-06 until:2022-12-07').get_items()):
    if i>20:
        break
...
# Using TwitterSearchScraper to scrape data and append tweets to list
for i,tweet in enumerate(sntwitter.TwitterSearchScraper('bitcoin since:2022-09-01 until:{today}').get_items()):
    if i>maxTweets:
        break
...
tweets_to_df.to_csv('tweets_bitcoin.csv') #write dataframe into csv file
savedTweets = pd.read_csv('tweets_bitcoin.csv',index_col=0) #reads csv file
